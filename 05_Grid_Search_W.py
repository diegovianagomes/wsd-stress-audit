...
#%%
# TESTAR O MODELO COM OS MELHORES PARAMETROS
...
for fold in folds:
    ...
    bin = pd.DataFrame(predicoes_binarias)

    # AVALIAÇÃO - PREDICT
    # SALVA TEMPO DE PREDICT
    start = time.time()
    predicoes_proba = model.predict_proba(x_val)
    end = time.time()
    runtime_proba = end - start  

    proba = pd.DataFrame(predicoes_proba)              
    ...
    roc_auc = roc_auc_score(y_val, proba[1])
    precision_curve, recall_curve, _ = precision_recall_curve(y_val, proba[1])
    auprc = auc(recall_curve, precision_curve)
    f1 = f1_score(y_val, bin)
    precision = precision_score(y_val, bin)
    recall = recall_score(y_val, bin)
    precision = precision_score(y_val, predicoes_binarias)
    recall = recall_score(y_val, predicoes_binarias)
